...
- **그동안 읽고 버리던 상태 필드를 엔티티로 엽니다.** `NR-67D` 가 보내는 44개
  ...
  것만 만듭니다.
  - **외기 온도** — 0.1℃ 단위. 실측으로 260 → 270 이 오전 내내 오르고 같은 시각
    동네 기상 관측이 25.4℃ 였습니다. `outsideTemperatureDisplayUse` 를 조건으로
    쓰지 않습니다. status 의 `*Use` 는 1=끔·2=켬이지만 feature 쪽 같은 이름은
    뜻이 달라서, 온수를 분명히 쓰는 이 기기도 `hotWaterUse` 가 1 로 옵니다.
    근거 없는 플래그로 엔티티를 막는 실수는 `powerCtrl` 에서 이미 했습니다.

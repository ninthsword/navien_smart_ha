...
#
# **라돈은 표에 남기되 값을 믿지 않는다.** 제보자의 NRT-20D 에는 라돈 센서가
# 없는 것으로 확인됐다. 그런데도 서버는 `radon` 과 `radonStageValue` 를 0 으로
# 계속 준다 — 「센서 없음」과 「0 Bq/㎥」를 구분해 주지 않는다. 항목을 지우지는
# 않는다. 다른 개체에는 센서가 있을 수 있고, 지우면 그 기기의 값을 버리게 된다.
# 항상 0 만 보인다면 센서가 없는 것이니 HA 에서 해당 엔티티를 숨기면 된다.
